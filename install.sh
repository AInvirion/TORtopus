...
configure_ssh() {
    ...
    # Warn about port preservation
    if [[ "$current_ssh_port" != "22" ]]; then
        warning "═══════════════════════════════════════════════════════"
        warning "PRESERVING NON-STANDARD SSH PORT: $current_ssh_port"
        warning "Your SSH port will remain unchanged"
        warning "═══════════════════════════════════════════════════════"
        sleep 2
    fi

    # Apply hardening (using detected port)
    cat > "$ssh_config" << EOF
